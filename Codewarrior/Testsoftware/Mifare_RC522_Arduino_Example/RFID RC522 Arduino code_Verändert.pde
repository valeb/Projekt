 ...
 */
uchar MFRC522_Request(uchar reqMode, uchar *TagType)  //PIC_REQALL, Array[16]
{
  uchar status;  
...
//		PCD_TRANSCEIVE, Array[16], 	   1, 	        Array[16],     backBits						
{
  uchar status = MI_ERR;
  uchar irqEn = 0x00;
  uchar waitIRq = 0x00;
  ...
  //write data into FIFO  //Write 1x PIC_REQALL
  for (i=0; i<sendLen; i++)
  {   
  ...
  if (command == PCD_TRANSCEIVE)  //Allways!!
  {    
